...
CLASS lhc_Sales IMPLEMENTATION.
  ...
  METHOD create.
    ...
    "passando em cada linha da entidade
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<lf_sales>).

      DATA(lo_uuid) = cl_uuid_factory=>create_system_uuid(  ).
      "instanciando a classe de buffer
      DATA(lo_instance) = zcl_gg_sales=>get_instance(  ).
      DATA(ls_sales) = CORRESPONDING ztgg_sales_un( <lf_sales> ).

      ls_sales-orderid = lo_uuid->create_uuid_x16(  ).
      APPEND ls_sales TO zcl_gg_sales=>gt_sales.
  ...
  METHOD update.
    ...
    TYPES: tt_sales   TYPE TABLE OF ztgg_sales_un WITH DEFAULT KEY,
           tt_sales_x TYPE TABLE OF zsgg_sales_un WITH DEFAULT KEY.

    DATA(lt_sales) = CORRESPONDING tt_sales( entities MAPPING FROM ENTITY ).
    "VAI PREENCHER COM X OS CAMPOS QUE TIVEREM UPDATE
    DATA(lt_sales_x) = CORRESPONDING tt_sales_x( entities MAPPING FROM ENTITY USING CONTROL ).

    IF lt_sales IS NOT INITIAL.

      SELECT FROM ztgg_sales_un
      FIELDS
          client,
          company,
          customer,
          orderid,
          vendor
          "para todos que tiveram entradas na minha empity
      FOR ALL ENTRIES IN @entities
      WHERE orderid = @entities-orderid
      INTO TABLE @DATA(lt_sales_old).

      zcl_gg_sales=>gt_sales = VALUE #( FOR ls IN lt_sales
...
CLASS lsc_ZI_GG_SALES DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZI_GG_SALES IMPLEMENTATION.
  ...
  METHOD save.
    ...
    MODIFY ztgg_sales_un from TABLE  @zcl_gg_sales=>gt_sales.

    clear zcl_gg_sales=>gt_sales.
